...
def load_corpus_into_tfidf(corpus):
    ...
    tf = TfidfVectorizer(analyzer='word', ngram_range=(
        1, 3), min_df=0.0, stop_words='english')
...
def find_similar_tfidf(tfidf_matrix, corpus_index):
    ...
    cosine_similarities = linear_kernel(
        tfidf_matrix[corpus_index:corpus_index + 1], tfidf_matrix).flatten()
    related_docs_indices = [i for i in cosine_similarities.argsort()[
        ::-1] if i != corpus_index]

    # return the matches (best matches to worst)
    tuple_vec = [(index, cosine_similarities[index])
...
def find_similar_labels(manga1, labels_vec, labels_weights, manga_data):
    ...
    for idx, manga2 in enumerate(manga_data):
        ...
        dic_matches[idx] = float(count_weighted) / \
            max(count_manga1_labels, count_manga2_labels)
...
def is_exact_match(manga1, manga2):
    ...
    for label in manga2.genre:
        if label in exact_matches_genre and label not in manga1.genre:
            exact_match = False
    for label in manga1.tags:
        if label in exact_matches_theme and label not in manga2.tags:
            exact_match = False
    for label in manga2.tags:
        if label in exact_matches_theme and label not in manga1.tags:
            exact_match = False
    return exact_match
